...
router.post('/products', authMiddleware, upload.array('images', 10), async (req, res) => {
  try {
    ...
    const normalizedIsFragile = ['true', '1', 'on'].includes(String(isFragile).toLowerCase());
    const normalizedIsHighValue = ['true', '1', 'on'].includes(String(isHighValue).toLowerCase());
    ...
    try {
      if (shouldUseDemoData()) {
        const product = {
          ...
          isFragile: normalizedIsFragile,
          isHighValue: normalizedIsHighValue,
