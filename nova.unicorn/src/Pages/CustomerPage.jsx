...
const CustomerPage = () => {
  ...
  return (
    <main className="min-h-screen bg-[#040d19] text-slate-100">
      <header className="bg-[#081827] shadow-[0_4px_0_rgba(59,130,246,0.25)]">
        <div className="mx-auto flex max-w-7xl flex-col gap-3 px-4 py-4 text-white sm:px-6 lg:px-8">
          <Link to="/customer" className="flex items-center gap-3"><span className="flex h-11 w-11 items-center justify-center rounded-xl bg-sky-300 text-2xl text-[#081827]"><FiShoppingBag /></span><span className="text-2xl font-black uppercase leading-[0.8] tracking-[-0.06em]">Nova<br />unicorn</span></Link>
          ...
          <div className="relative w-full"><FiSearch className="absolute left-4 top-1/2 -translate-y-1/2 text-sky-200" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search products..." aria-label="Search products" className="w-full border-2 border-sky-500/20 bg-[#0d182c] px-12 py-3 text-sm text-slate-100 outline-none placeholder:text-sky-100/50 focus:border-sky-400" /></div>
        </div>
        <nav className="border-t border-sky-500/20" aria-label="Store navigation"><div className="mx-auto grid max-w-7xl grid-cols-4 gap-1 px-4 py-2 sm:grid-cols-8 sm:px-6 lg:px-8">
      ...
      <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        <section className="store-hero relative overflow-hidden bg-gradient-to-r from-[#081827] via-[#11233d] to-[#1d4ed8] px-6 py-10 text-white sm:px-12 sm:py-16"><div className="relative z-10 max-w-3xl"><p className="font-serif text-3xl italic text-sky-200/80 sm:text-5xl">Welcome to NovaUnicorn</p><h1 className="mt-3 bg-gradient-to-r from-sky-50 via-blue-100 to-blue-300 bg-clip-text text-4xl font-black uppercase leading-[0.95] tracking-tight text-transparent sm:text-7xl">A market filled with unlimited opportunities</h1><p className="mt-5 max-w-2xl text-sm font-medium leading-6 text-sky-100/80 sm:text-base">Discover trusted vendors, explore new categories, and find something made for your everyday life.</p><Link to="/category/shopvendor" className="mt-7 inline-flex items-center gap-2 bg-sky-300 px-6 py-3 text-sm font-black uppercase text-[#081827] hover:bg-sky-200">Start exploring <FiArrowRight /></Link></div><div className="absolute -right-10 bottom-[-5rem] h-72 w-72 rounded-full bg-sky-300/10 sm:right-12 sm:h-96 sm:w-96" /><div className="absolute right-16 top-12 h-28 w-28 rounded-full border-[18px] border-sky-200/20 sm:right-40 sm:h-44 sm:w-44" /></section>
        <div className="mt-5 grid gap-4 md:grid-cols-3"><Link to="/category/shopvendor" className="promo-tile bg-[#0c1727]">Shop vendor <span>Everyday finds</span><FiArrowRight /></Link><Link to="/category/cardealer" className="promo-tile bg-[#11233d]">Car dealer <span>Drive something great</span><FiArrowRight /></Link><Link to="/category/blackmarket" className="promo-tile bg-[#193055]">Blackmarket <span>Unique offers</span><FiArrowRight /></Link></div>
        <section className="mt-10"><div className="flex flex-col justify-between gap-3 border-b-2 border-sky-500/30 pb-3 sm:flex-row sm:items-end"><div><p className="text-xs font-black uppercase tracking-[0.25em] text-sky-300">Vendor marketplace</p><h2 className="mt-1 text-3xl font-black uppercase text-sky-50">Latest arrivals</h2></div><span className="text-sm font-semibold text-sky-100/70">{visibleProducts.length} products available</span></div>
