...
const VendorPreRegistrationPage = () => {
  ...
  return (
    <main className="min-h-screen bg-[#02070d] px-4 py-8 text-[#f3f5f7] sm:px-8 lg:px-12">
      {showCelebration && <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#02070d]/80 px-4" role="status" aria-live="polite"><div className="relative w-full max-w-lg overflow-hidden rounded-3xl bg-[#101822] p-8 text-center shadow-2xl sm:p-12"><div className="pointer-events-none absolute inset-0" aria-hidden="true">{Array.from({ length: 28 }, (_, index) => <span key={index} className="absolute h-3 w-2 animate-bounce rounded-sm" style={{ left: `${(index * 37) % 100}%`, top: `${(index * 19) % 75}%`, backgroundColor: ["#7ce6d4", "#f59e0b", "#ec4899", "#06b6d4", "#6366f1"][index % 5], transform: `rotate(${index * 29}deg)`, animationDelay: `${(index % 7) * 80}ms` }} />)}<span className="absolute left-8 top-8 h-20 w-16 rounded-[50%] bg-[#7ce6d4] shadow-lg" /><span className="absolute right-8 top-12 h-24 w-16 rounded-[50%] bg-[#a7f3d0] shadow-lg" /><span className="absolute bottom-8 left-1/2 h-20 w-14 rounded-[50%] bg-[#dfe7ee] shadow-lg" /></div><div className="relative"><p className="text-5xl">Thank you!</p><h2 className="mt-4 text-3xl font-black text-[#7ce6d4]">Thank you for registering</h2><p className="mt-3 text-sm leading-6 text-[#dfe7ee]">Your shop has been added to the NovaUnicorn vendor interest list.</p><button type="button" onClick={() => setShowCelebration(false)} className="mt-7 rounded-xl bg-[#7ce6d4] px-6 py-3 text-sm font-bold text-[#071118] hover:bg-[#60e0c4]">Continue</button></div></div></div>}
      <div className="mx-auto max-w-7xl overflow-hidden rounded-[2rem] bg-[#0b1118] shadow-2xl">
        <section className="relative overflow-hidden bg-[#101822] px-6 py-16 text-white sm:px-12 sm:py-24 lg:px-20 lg:py-32">
