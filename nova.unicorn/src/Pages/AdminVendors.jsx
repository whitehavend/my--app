...
import { FiCheckCircle, FiClock, FiFileText, FiMapPin, FiRefreshCw, FiSearch, FiShield, FiXCircle } from "react-icons/fi";
...
const AdminVendors = () => {
  ...
  const [officerForm, setOfficerForm] = useState({ firstName: "", secondName: "", username: "", email: "", password: "" });
  const [officerStatus, setOfficerStatus] = useState("");
  const [officerError, setOfficerError] = useState("");
  const [logisticForm, setLogisticForm] = useState({ firstName: "", secondName: "", username: "", email: "", password: "", phoneNumber: "", countryCode: "" });
  ...
  useEffect(() => {
    if (user?.role === "admin") {
      ...
      loadQueue();
      fetch(`${process.env.REACT_APP_BASEURL || "http://localhost:5001/api"}/auth/vendor-pre-registrations`, { headers: headers() })
        .then((response) => response.json())
        .then((body) => { setPreRegistrationCount(body.count || 0); setPreRegistrations(body.registrations || []); })
        .catch(() => {});
    }
  }, [dispatch, user]);
  ...
  const createCollectionOfficer = async (event) => {
    event.preventDefault();
    setOfficerStatus("Creating account...");
    setOfficerError("");
    try {
      const response = await fetch(`${process.env.REACT_APP_BASEURL || "http://localhost:5001/api"}/auth/collection-officers`, {
        method: "POST",
        headers: { "Content-Type": "application/json", ...headers() },
        body: JSON.stringify(officerForm),
      });
      const body = await response.json();
      if (!response.ok) throw new Error(body.error || "Unable to create collection officer account");
      setOfficerForm({ firstName: "", secondName: "", username: "", email: "", password: "" });
      setOfficerStatus("Account created. The collection officer can now sign in from the login page.");
    } catch (createError) {
      setOfficerStatus("");
      setOfficerError(createError.message);
    }
  };

  const createLogistic = async (event) => {
    event.preventDefault();
    setLogisticStatus("Creating account...");
    setLogisticError("");
    try {
      const response = await fetch(`${process.env.REACT_APP_BASEURL || "http://localhost:5001/api"}/auth/logistics`, {
        method: "POST",
        headers: { "Content-Type": "application/json", ...headers() },
        body: JSON.stringify(logisticForm),
      });
      const body = await response.json();
      if (!response.ok) throw new Error(body.error || "Unable to create logistic account");
      setLogisticForm({ firstName: "", secondName: "", username: "", email: "", password: "", phoneNumber: "", countryCode: "" });
      setLogisticStatus("Account created. The logistic can now sign in from the login page.");
    } catch (createError) {
      setLogisticStatus("");
      setLogisticError(createError.message);
    }
  };
  ...
  return <main className="min-h-screen bg-[#02070d] px-4 py-8 sm:px-6 lg:px-10"><div className="mx-auto max-w-7xl">
    <section className="relative overflow-hidden rounded-[2rem] bg-[#101822] px-6 py-9 text-white shadow-xl sm:px-10"><div className="absolute -right-16 -top-24 h-64 w-64 rounded-full border-[36px] border-[#7ce6d4]/20" /><div className="relative flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.3em] text-[#7ce6d4]">Nova operations desk</p><h1 className="mt-3 max-w-2xl text-4xl font-black sm:text-5xl">Trust, reviewed with care.</h1><p className="mt-4 max-w-xl text-sm leading-6 text-[#dfe7ee]">Approve sellers, inspect submitted evidence, and keep every verification decision visible.</p></div><Link to="/login" className="inline-flex shrink-0 items-center justify-center rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm font-bold text-white hover:bg-white/10">Login page</Link></div></section>
    <section className="mt-6 rounded-2xl bg-white p-5 shadow-sm sm:p-6"><div><p className="text-xs font-bold uppercase tracking-[0.2em] text-emerald-700">Access management</p><h2 className="mt-1 text-2xl font-bold text-gray-900">Create a collection officer account</h2><p className="mt-2 text-sm text-gray-500">Collection officers use this account to sign in to the separate order and logistics workspace.</p></div><form onSubmit={createCollectionOfficer} className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-5"><input required value={officerForm.firstName} onChange={(event) => setOfficerForm({ ...officerForm, firstName: event.target.value })} placeholder="First name" className="rounded-xl border border-gray-200 p-3 text-sm" /><input required value={officerForm.secondName} onChange={(event) => setOfficerForm({ ...officerForm, secondName: event.target.value })} placeholder="Second name" className="rounded-xl border border-gray-200 p-3 text-sm" /><input required value={officerForm.username} onChange={(event) => setOfficerForm({ ...officerForm, username: event.target.value })} placeholder="Username" className="rounded-xl border border-gray-200 p-3 text-sm" /><input required type="email" value={officerForm.email} onChange={(event) => setOfficerForm({ ...officerForm, email: event.target.value })} placeholder="Email" className="rounded-xl border border-gray-200 p-3 text-sm" /><input required type="password" minLength="6" value={officerForm.password} onChange={(event) => setOfficerForm({ ...officerForm, password: event.target.value })} placeholder="Temporary password" className="rounded-xl border border-gray-200 p-3 text-sm" /><button type="submit" className="rounded-xl bg-emerald-700 px-4 py-3 text-sm font-bold text-white hover:bg-emerald-800 sm:col-span-2 lg:col-span-5 lg:justify-self-start">Create collection officer</button></form>{officerStatus && <p className="mt-3 rounded-xl bg-emerald-50 p-3 text-sm text-emerald-700">{officerStatus}</p>}{officerError && <p className="mt-3 rounded-xl bg-red-50 p-3 text-sm text-red-700">{officerError}</p>}</section>
    <section className="mt-6 rounded-2xl bg-white p-5 shadow-sm sm:p-6"><div><p className="text-xs font-bold uppercase tracking-[0.2em] text-emerald-700">Access management</p><h2 className="mt-1 text-2xl font-bold text-gray-900">Create a logistic account</h2><p className="mt-2 text-sm text-gray-500">Logistics are created by administrators and then sign in with the credentials provided.</p></div><form onSubmit={createLogistic} className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><input required value={logisticForm.firstName} onChange={(event) => setLogisticForm({ ...logisticForm, firstName: event.target.value })} placeholder="First name" className="rounded-xl border border-gray-200 p-3 text-sm" /><input required value={logisticForm.secondName} onChange={(event) => setLogisticForm({ ...logisticForm, secondName: event.target.value })} placeholder="Second name" className="rounded-xl border border-gray-200 p-3 text-sm" /><input required value={logisticForm.username} onChange={(event) => setLogisticForm({ ...logisticForm, username: event.target.value })} placeholder="Username" className="rounded-xl border border-gray-200 p-3 text-sm" /><input required type="email" value={logisticForm.email} onChange={(event) => setLogisticForm({ ...logisticForm, email: event.target.value })} placeholder="Email" className="rounded-xl border border-gray-200 p-3 text-sm" /><input required type="password" minLength="6" value={logisticForm.password} onChange={(event) => setLogisticForm({ ...logisticForm, password: event.target.value })} placeholder="Temporary password" className="rounded-xl border border-gray-200 p-3 text-sm" /><input required value={logisticForm.countryCode} onChange={(event) => setLogisticForm({ ...logisticForm, countryCode: event.target.value })} placeholder="Country code e.g. +254" className="rounded-xl border border-gray-200 p-3 text-sm" /><input required value={logisticForm.phoneNumber} onChange={(event) => setLogisticForm({ ...logisticForm, phoneNumber: event.target.value })} placeholder="Phone number" className="rounded-xl border border-gray-200 p-3 text-sm" /><button type="submit" className="rounded-xl bg-emerald-700 px-4 py-3 text-sm font-bold text-white hover:bg-emerald-800">Create logistic</button></form>{logisticStatus && <p className="mt-3 rounded-xl bg-emerald-50 p-3 text-sm text-emerald-700">{logisticStatus}</p>}{logisticError && <p className="mt-3 rounded-xl bg-red-50 p-3 text-sm text-red-700">{logisticError}</p>}</section>
    <section className="mt-6 grid gap-4 sm:grid-cols-4"><Metric icon={<FiClock />} label="Account approvals" value={pendingVendors.length} note="Waiting for a decision" /><Metric icon={<FiFileText />} label="Manual documents" value={pendingDocuments} note="Need team review" /><Metric icon={<FiShield />} label="Verification files" value={vendors.length} note="Submitted vendor records" /><Metric icon={<FiMapPin />} label="Shop preregistrations" value={preRegistrationCount} note="Interest list" /></section>
    <section className="mt-6 rounded-2xl bg-white p-5 shadow-sm sm:p-6"><p className="text-xs font-bold uppercase tracking-[0.2em] text-emerald-700">Vendor pipeline</p><h2 className="mt-1 text-2xl font-bold text-gray-900">Preregistered shops</h2><div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{preRegistrations.length ? preRegistrations.map((registration) => <div key={registration._id} className="rounded-xl border border-gray-100 bg-gray-50 p-3 text-sm font-semibold text-gray-800">{registration.shopName}<span className="mt-1 block text-xs font-normal text-gray-500">{new Date(registration.createdAt).toLocaleDateString()}</span></div>) : <p className="text-sm text-gray-500">No preregistered shops yet.</p>}</div></section>
