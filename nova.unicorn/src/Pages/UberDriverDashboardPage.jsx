...
const UberDriverDashboardPage = () => {
  ...
  const stats = useMemo(
    () => [
      { label: "Today's rides", value: "0", icon: <FaBolt className="text-[#c4c8cc]" /> },
      { label: "Earnings", value: "KSh 0", icon: <FaWallet className="text-[#c4c8cc]" /> },
      { label: "Verified", value: "KYC OK", icon: <FaShieldAlt className="text-[#c4c8cc]" /> },
    ],
    []
  );
    ...
    <main className="min-h-screen bg-[#02070d] text-[#f3f5f7]">
      <header className="border-b border-white/10 bg-[#0b0c0d]">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-5 sm:px-6 lg:px-8">
          <div className="flex items-center gap-3 text-[#f3f5f7]">
            <span className="flex h-11 w-11 items-center justify-center rounded-xl bg-[#c4c8cc] text-xl text-[#202225]">
              <FaUber />
            </span>
            <div>
              <p className="text-xs font-black uppercase tracking-[0.3em] text-[#c4c8cc]">Nova driver</p>
              <p className="text-lg font-black uppercase leading-[0.9] tracking-[-0.05em]">Driver dashboard</p>
            </div>
          </div>
          <Link to="/account" className="rounded-full border border-white/15 bg-white/5 px-4 py-2 text-xs font-bold uppercase tracking-[0.2em] text-[#c4c8cc]">
            My account
          </Link>
        </div>
      </header>
      ...
      <div className="mx-auto max-w-6xl space-y-8 px-4 py-8 sm:px-6 lg:px-8">
        <section className="rounded-3xl border border-white/10 bg-gradient-to-r from-[#02070d] via-[#17191c] to-[#24272a] p-6 shadow-[0_20px_60px_rgba(0,0,0,0.35)] sm:p-8">
          <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.28em] text-[#c4c8cc]">Welcome aboard</p>
            ...
            {stats.map((stat) => (
              <div key={stat.label} className="rounded-2xl border border-white/10 bg-white/5 p-4">
                <div className="flex items-center justify-between">
                  <span className="text-sm text-[#c4c8cc]">{stat.label}</span>
        ...
        <section className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
          <div className="rounded-3xl border border-white/10 bg-[#111315] p-6">
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-xs font-black uppercase tracking-[0.25em] text-[#c4c8cc]">Live route</p>
                <h2 className="mt-2 text-2xl font-black text-white">Pin your current pickup point</h2>
              </div>
              <FaMapMarkerAlt className="text-2xl text-[#c4c8cc]" />
            </div>

            <button
              type="button"
              onClick={pinCurrentLocation}
              className="mt-5 w-full rounded-xl bg-[#c4c8cc] px-4 py-3 text-sm font-black uppercase text-[#202225] hover:bg-[#e3e5e7]"
            >
              Share my live location
            </button>

            {liveLocation ? (
              <div className="mt-5 rounded-2xl border border-white/10 bg-white/5 p-4 text-sm text-[#d1d3d5]">
                <p>Latitude: {liveLocation.latitude.toFixed(5)}</p>
                <p>Longitude: {liveLocation.longitude.toFixed(5)}</p>
                <a href={googleMapsUrl} target="_blank" rel="noreferrer" className="mt-3 inline-block font-semibold text-[#c4c8cc] underline underline-offset-4">
                  Open in Google Maps
                </a>
              </div>
            ) : (
              <p className="mt-5 text-sm text-[#a8b0bb]">Your live pickup location is not shared yet. Use the button above to enable it.</p>
            )}
          ...
          <div className="rounded-3xl border border-white/10 bg-[#111315] p-6">
            <p className="text-xs font-black uppercase tracking-[0.25em] text-[#c4c8cc]">Driver profile</p>
            <h2 className="mt-2 text-2xl font-black text-white">Account status</h2>

            <div className="mt-5 space-y-3 text-sm text-[#c4c8cc]">
              <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                <p className="font-semibold text-[#f3f5f7]">Verification</p>
                <p className="mt-2">KYC verified automatically</p>
              </div>
              <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                <p className="font-semibold text-[#f3f5f7]">Driver license</p>
        ...
        <section className="rounded-3xl border border-white/10 bg-[#111315] p-6">
          <p className="text-xs font-black uppercase tracking-[0.25em] text-[#c4c8cc]">Ride queue</p>
          <h2 className="mt-2 text-2xl font-black text-white">Current trip opportunities</h2>

          <div className="mt-6 rounded-2xl border border-dashed border-white/20 bg-[#17191c] p-8 text-center">
            <p className="text-xl font-black text-[#f3f5f7]">No rides in queue yet</p>
            <p className="mt-3 text-sm text-[#a8b0bb]">When a genuine customer requests a ride, it will appear here automatically.</p>
          </div>
        </section>
      </div>
    </main>
