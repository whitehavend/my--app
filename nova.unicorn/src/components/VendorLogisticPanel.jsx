        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2">
          {logistics.map((logistic) => (
            <article key={logistic.id || logistic.uid} className="rounded-xl border border-emerald-200 bg-white p-4">
              ...
              <button type="button" onClick={() => dispatch(requestLogistic(logistic.id || logistic.uid))} className="mt-3 rounded-md bg-emerald-700 px-4 py-2 text-sm font-semibold text-white hover:bg-emerald-800">
                Request pickup
              </button>
            </article>
          ))}
